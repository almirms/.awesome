...
function next_client_total(i, c)
     if awful.layout.getname() == "max" then
         local cls = client.get()
         for k, c in pairs(cls) do
             if string.find(c.class,"jetbrains") and c.minimized then
                c.minimized = false
             end
         end
     end
...
-- Calendar
lain.widgets.calendar.attach(mytextclock, { font_size = 9, font="SourceCodePro" })
...
awful.screen.connect_for_each_screen(function(s)
    ...
    awful.tag({ "1", "2", "3", "4", "5", "6", "=\\", "8", "9" }, s, awful.layout.layouts[1])
...
globalkeys = gears.table.join(
    ...
    awful.key({}, "#121",
        function ()
            os.execute(string.format("amixer -D pulse set Master toggle", volumewidget.channel))
            volumewidget.update()
        end),
...
awful.rules.rules = {
    ...
    --gambis intellij
    { rule = { class = "jetbrains-%w+",instance= "sun-awt-X11-XFramePeer" },
       callback = function(c)
           c:connect_signal("focus", function (cc)
               if awful.layout.getname() == "max" then
                   -- make other jetbrains windows minimum
                   local cls = client.get()
                   for _, t in ipairs(cls) do
                     if string.find(t.class, "jetbrains")  ~= nil then
                         if t.name ~= cc.name then
                             t.minimized = true
                         end
                     end
                   end
